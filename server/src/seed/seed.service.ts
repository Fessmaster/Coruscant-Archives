...
export class SeedService implements OnApplicationBootstrap {
  ...
  async createRelations<T extends ObjectLiteral>(
    ...
    entityName: string,
    relationMap: Map<number, Record<string, number | number[]>> | null,
  ) {
    const entitiesToSave: T[] = [];
    ...
    if (relationMap == null) throw new Error('Relation Map not exist');
  ...
  async seedingFromApi() {
    const people = await this.peopleRepository.find();
    if (people.length > 0) {
      this.logger.log(
        '📃 The data was added earlier and seeding will be skipped',
      );     
      return;
    }
    try {
      const configData = this.getSeedConfig();
      for (const config of configData) {
        await this.seedData(
          config.url,
          config.numberFields,
          config.extraFields,
          config.repository as Repository<ObjectLiteral>,
          config.relationMap,
        );
      }
      ...
      const configToSetRelation = configData.filter(
        (config) => config.shouldCreateRelations,
      );
      for (const config of configToSetRelation) {
